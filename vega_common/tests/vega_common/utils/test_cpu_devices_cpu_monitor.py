...
MOCK_TEMPS_MISSING_CURRENT = {
    "coretemp": [
        psutil._common.shwtemp(
            label="Package id 0", current=None, high=85.0, critical=100.0
        ),  # Missing 'current'
    ]
}
...
class TestCpuMonitor:
    ...
    def test_update_status_multiple_devices(self, cpu_monitor, mock_psutil):
        ...
        assert (
            abs(status.get_property("temperature") - 54.0) < 1e-6
        )  # k10temp/Tdie should be selected
        assert not status.has_error("temperature")
    ...
    def test_custom_sensor_label_priority(self, mock_psutil):
        ...
        monitor = CpuMonitor(
            device_id="custom_cpu", cpu_temp_sensor_labels=["tccd1", "tdie", "package"]
        )
    ...
    def test_non_preferred_device_with_preferred_label(self, mock_psutil):
        ...
        mock_data = {
            "non_preferred_device": [
                psutil._common.shwtemp(
                    label="Package id 0", current=72.0, high=85.0, critical=100.0
                ),
            ]
        }
        ...
        assert "Could not find a suitable CPU temperature sensor" in monitor.status.get_error(
            "temperature"
        )
    ...
    def test_label_normalization(self, mock_psutil):
        ...
        mock_data = {
            "k10temp": [
                ...
                psutil._common.shwtemp(
                    label="PACKAGE ID 0", current=59.0, high=None, critical=None
                ),
            ]
        }
        mock_psutil.sensors_temperatures.return_value = mock_data
        ...
        monitor = CpuMonitor(
            device_id="normalize_cpu", cpu_temp_sensor_labels=["package_id_0", "core_0"]
        )
        monitor.update_status()
    ...
    def test_temperature_history_tracking(self, cpu_monitor, mock_psutil):
        ...
        new_data = {
            "coretemp": [
                psutil._common.shwtemp(
                    label="Package id 0", current=57.0, high=85.0, critical=100.0
                ),
            ]
        }
    ...
    def test_extreme_temperature_values(self, mock_psutil):
        ...
        mock_high = {
            "coretemp": [
                psutil._common.shwtemp(
                    label="Package id 0", current=999.0, high=None, critical=None
                ),
            ]
        }
        ...
        mock_low = {
            "coretemp": [
                psutil._common.shwtemp(
                    label="Package id 0", current=-50.0, high=None, critical=None
                ),
            ]
        }
    ...
    def test_sorting_by_label_priority(self, mock_psutil):
        ...
        monitor = CpuMonitor(
            device_id="sort_priority_cpu", cpu_temp_sensor_labels=["tdie", "tctl", "tccd1"]
        )
        monitor.update_status()
    ...
    def test_non_preferred_device_only(self, mock_psutil):
        ...
        monitor = CpuMonitor(
            device_id="non_preferred_device_cpu", cpu_temp_device_names=["k10temp", "coretemp"]
        )
        monitor.update_status()
