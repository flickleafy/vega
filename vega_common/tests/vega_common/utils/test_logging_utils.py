...
class TestSetupLogging:
    ...
    def test_setup_logging_handlers(self):
        ...
        with temporary_log_file() as log_file:
            with patch("logging.getLogger") as mock_get_logger:
                ...
                handlers = [
                    call_args[0][0] for call_args in mock_root_logger.addHandler.call_args_list
                ]

                # Verify one is a StreamHandler (for console)
                assert any(
                    isinstance(h, logging.StreamHandler) or "StreamHandler" in str(type(h))
                    for h in handlers
                )
            ...
            with patch("os.makedirs") as mock_makedirs:
                with patch("logging.handlers.RotatingFileHandler") as mock_handler:
                    mock_handler.return_value = (
                        MagicMock()
                    )  # Return a mock handler that won't cause issues
    ...
    @pytest.mark.skipif(
        os.name != "posix", reason="Permission test only applicable on POSIX systems"
    )
    ...
    def test_setup_logging_error_handling(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch(
                "traceback.extract_stack", return_value=[("not_a_test_file", 0, "function", None)]
            ):
...
class TestTaskContext:
    ...
    def test_thread_local_isolation(self):
        ...
        threads = [threading.Thread(target=worker_thread, args=(f"thread-{i}",)) for i in range(2)]
...
class TestThreadSafety:
    ...
    def test_concurrent_logging(self, mock_info):
        ...
        with temporary_log_file() as log_file:
            ...
            threads = [threading.Thread(target=logging_worker, args=(i,)) for i in range(3)]
...
class TestAdvancedThreading:
    ...
    def test_task_context_with_thread_pool(self):
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            futures = [executor.submit(get_context_in_thread, f"thread-{i}") for i in range(5)]
    ...
    def test_concurrent_context_changes(self):
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(change_context_repeatedly, i) for i in range(3)]
            ...
            try:
                ...
                try:
                    ...
                    file_handler = logging.handlers.RotatingFileHandler(
                        log_path, maxBytes=small_size, backupCount=3
                    )
