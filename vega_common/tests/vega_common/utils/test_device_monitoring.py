...
class TestDeviceStatus:
    ...
    def test_tracking_properties(self):
        ...
        # Check average
        assert status.get_property_average("temperature") == 10.5  # (7*0.0 + 30.0 + 35.0 + 40.0) / 10 = 10.5
...
class MockDeviceController(DeviceController):
    ...
    def get_available_settings(self):
        ...
        return {
            "fan_speed": {
                "current": self.current_settings["fan_speed"],
                "min": 0,
                "max": 100
            },
            "power_limit": {
                "current": self.current_settings["power_limit"],
                "min": 50,
                "max": 150
            }
        }
...
class TestDeviceManager:
    ...
    def test_apply_device_settings(self):
        ...
        result = self.manager.apply_device_settings("gpu", "gpu-1", {
            "fan_speed": 80
        })
        assert result is True
