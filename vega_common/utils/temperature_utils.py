...
def estimate_cpu_from_liquid_temp(
    liquid_temp: float, offset: float = 15, scaling_factor: float = 1.4, mode: int = 1
) -> float:
    """
...
def temperature_within_range(temp: float, target: float, tolerance: float = 2.0) -> bool:
    """
...
def classify_temperature(temp: float, ranges: List[Tuple[float, float, str]] = None) -> str:
    """
