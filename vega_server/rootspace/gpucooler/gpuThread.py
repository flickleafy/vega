            ...
            try:
                ...
                for i in range(device_count):
                    try:
                        ...
                        gpu_temp_windows[monitor.device_id] = NumericSlidingWindow(size=TEMPERATURE_WINDOW_SIZE)
                        gpu_controllers[monitor.device_id] = controller # Store controller for easy access
                        
                        logging.info(f"Registered monitor and controller for GPU {i} (ID: {monitor.device_id})")

                    except (NVMLError, ValueError) as e:
                        logging.error(f"Failed to initialize monitor/controller for GPU {i}: {e}", exc_info=True)
        ...
        while True:
            ...
            for status in gpu_statuses:
                ...
                if gpu_temp is None or status.is_error("temperature"):
                    logging.warning(f"No valid temperature reading for GPU {device_id}. Skipping control.")
                ...
                if controller:
                    try:
                        ...
                        logging.error(f"Error setting fan speed for GPU {device_id}: {e}", exc_info=True)
                else:
                ...
                logging.info(f"GPU {device_id}: Temp={gpu_temp:.1f}°C, AvgTemp={gpu_average_degree:.1f}°C, "
                ...
                globals.WC_DATA_OUT[0][f"gpu{gpu_index_str}_average_degree"] = round(gpu_average_degree, 1)
